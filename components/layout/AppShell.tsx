...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  const renderSidebarNav = () => (
    <>
      <div className="mb-8 flex items-center gap-2 px-2">
        ...
        <div>
          <div className="text-sm font-semibold tracking-wide">Risk Platform</div>
  ...
  return (
    <div className="flex min-h-screen bg-slate-950 text-slate-100">
      ...
      {/* Shared sidebar (mobile + desktop) */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-slate-800 bg-slate-950/95 px-4 py-6 backdrop-blur transition-transform duration-300 ease-in-out md:translate-x-0 ${
        ...
        <header className="flex h-14 items-center justify-between border-b border-slate-800 bg-slate-950/60 px-4 backdrop-blur">
          <div className="flex items-center gap-3">
            <button
              ...
              className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-slate-700 text-slate-200 hover:bg-slate-800 md:hidden"
