...
export default function DashboardPage() {
  const activeAlerts = mockAlerts.filter((a) => a.status !== "Closed").length;
  const highRiskPlayers = mockPlayers.filter(
    (p) => p.riskBand === "High" || p.riskBand === "Severe",
  ).length;
  const pendingCases = mockCases.filter(
    (c) => c.status === "New" || c.status === "Assigned" || c.status === "Investigating",
  ).length;
  ...
  return (
    <>
      <div className="flex items-baseline justify-between gap-4">
        ...
        <Badge variant="outline">Last refresh: simulated</Badge>
      </div>
